...
class RSIDivergenceStrategy(BaseStrategy):
    ...
    def __init__(self):
        ...
        # Evening tuning 2026-02-16: disable SELL signals in ranging regime.
        # When True, only emit SELL if a downtrend is confirmed (price below mean).
        self.require_trend_for_sell = os.getenv("RSI_REQUIRE_TREND_FOR_SELL", "true").lower() == "true"
    ...
    def detect(
        ...
        mean = sum(price_values) / len(price_values)  # used for trend detection
        gains = []
        losses = []
        ...
        # Sell when overbought — but require downtrend confirmation in ranging regime
        # (Evening tuning 2026-02-16: require_trend_for_sell suppresses SELL in ranging)
        elif rsi >= self.overbought:
            if self.require_trend_for_sell:
                # Confirm trend: price must be below the window mean (bearish bias)
                is_downtrend = price < mean
                if not is_downtrend:
                    return None
